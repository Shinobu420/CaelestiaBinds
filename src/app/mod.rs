...
impl KeybindsApp {
    ...
    fn load_logo_texture_if_needed(&mut self, ctx: &egui::Context) {
        if self.state.logo_texture.is_none() {
            let logo_bytes_light: &[u8] = include_bytes!("../../assets/Logo_W.png");
            let logo_bytes_dark: &[u8] = include_bytes!("../../assets/Logo_B.png");
            let logo_bytes: &[u8];
    
            if crate::ui::styling::css::is_dark_mode() {
                logo_bytes = logo_bytes_dark;
            }else {
                logo_bytes = logo_bytes_light;
            }
    ...
    fn handle_search_bar_focus(&self, ctx: &egui::Context) {
        ...
        if !self.state.flags.zen_mode
            && !search_bar_focused
            && ctx.input(|i| i.key_pressed(self.state.search_keybind))
        {
            ctx.memory_mut(|m| m.request_focus(egui::Id::new("search_bar")));
            ctx.input_mut(|i| {
                i.events
                    .retain(|e| !matches!(e, egui::Event::Text(s) if s == "#")&&!matches!(e, egui::Event::Text(s) if s == "/"));
            });
        }
    }

    fn apply_theme_or_css(&mut self, ctx: &egui::Context) {
        if crate::ui::styling::css::scheme_path().exists() {
            let path = crate::ui::styling::css::scheme_path();
            if let Ok(meta) = std::fs::metadata(&path)
                && let Ok(modified) = meta.modified()
            {
                let changed = self.state.last_css_mtime.is_none_or(|prev| modified > prev);
                if changed {
                    let _ = crate::ui::styling::css::apply_theme(ctx);
        ...
        if self.state.flags.show_options_window {
            ctx.show_viewport_immediate(
                ...
                egui::ViewportBuilder::default()
                    .with_title("CaelestiaBind – Options")
                    .with_app_id("CaelestiaBind_Options")
                    .with_resizable(true)
                ...
                |vctx, _class| {
                    ...
                    egui::CentralPanel::default().show(vctx, |ui| {
                        ...
                        let mut opts = crate::ui::options::OptionsState {
                            ...
                            search_keybind: &mut self.state.search_keybind,
                        };
                        crate::ui::options::render_options_contents(ui, &mut opts);
